...
let winCheckCondition = function (y) {
	if (y) {
			$('.game__result').hide();
			resultRenderShow();
			resultRender();
			...
			$('.container').empty()
						   .append('<div class="gamewin"> ВЫ ПОБЕДИЛИ! <br> КОЛИЧЕСТВО ПОПЫТОК: ' + tryCounter + '<br>' + 
						   	'<a href="file:///C:/Users/User3/Desktop/Gam/index.html?" class="btn btn__win-start"> Начать заново </a> ' + '</div>');
		};
};